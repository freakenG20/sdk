...
struct StandardClient : public MegaApp
{
    ...
    bool sync_syncable(Sync* sync, const char *name, string* localPath, Node*) override
    {
        return sync_syncable(sync, name, localPath);
    }

    bool sync_syncable(Sync*, const char *name, string* localPath) override
    {
        onCallback();
    ...
    void transfer_added(Transfer*) override { ++transfersAdded; onCallback(); }
    void transfer_removed(Transfer*) override { ++transfersRemoved; onCallback(); }
    void transfer_prepare(Transfer*) override { ++transfersPrepared; onCallback(); }
    void transfer_failed(Transfer*,  const Error&, dstime = 0) override { ++transfersFailed; onCallback(); }
    void transfer_update(Transfer*) override { ++transfersUpdated; onCallback(); }
    void transfer_complete(Transfer*) override { ++transfersComplete; onCallback(); }
    ...
    bool recursiveConfirm(Model::ModelNode* mn, LocalNode* n, int& descendants, const string& identifier, int depth, bool& firstreported)
    {
        ...
        if (localNodesMustHaveNodes)
        {
            if (n->node == nullptr)
            {
                cout << "dgw: lnmhv: lp = " << localpath << std::endl;
            }
        ...
        for (auto& n2 : n->children)
        {
            ...
            if (!(child.deleted || child.isIgnored()))
            {
...
public:
    ...
    fs::path debrisFilePath(const string& debrisName,
                            const string& path) const
    {
        return fs::path(debrisName) / todaysDate() / path;
    }
...
TEST_F(FilterFixture, FilterChangeWhileDownloading)
{
    ...
    // Set up cloud.
    {
        ...
        model.generate(root(*cu));

        // Log in client.
        ASSERT_TRUE(cu->login_reset_makeremotenodes("x"));

        // Add and start sync.
        ASSERT_TRUE(setupSync(*cu, "", "x"));
    ...
    cdu->mOnFileAdded =
      [&](File& file)
      {
          ...
          ASSERT_TRUE(createFile(root(*cdu) / ".megaignore",
                                 ignoreFile.data(),
                                 ignoreFile.size()));
      };
...
TEST_F(FilterFixture, NameFilter)
{
    ...
    localFS.addfile(".megaignore",
                    ...
                    "+N:*.nN\n"
                    // exclude all *.N* in the root.
                    "-N:*.N*\n"
                    // include all *.Ni in the root.
                    "+N:*.Ni\n");
...
{
public:
    fs::path debrisFilePath(const string& path) const
    {
        return fs::path("SyncDebris") / todaysDate() / path;
    }
...
TEST_F(LocalToCloudFilterFixture, DoesntRubbishIgnoredNodes)
{
    ...
    ASSERT_TRUE(fs::remove(root(*cu) / "fx"));
...
TEST_F(LocalToCloudFilterFixture, FilterDeferredChange)
{
    ...
    ASSERT_TRUE(fs::remove(root(*cu) / "1" / ".megaignore"));
    ...
    ASSERT_TRUE(fs::remove(root(*cu) / ".megaignore"));
...
TEST_F(LocalToCloudFilterFixture, FilterMovedAcrossHierarchy)
{
    ...
    fs::rename(root(*cu) / "0" / ".megaignore",
               root(*cu) / "1" / ".megaignore");
...
TEST_F(LocalToCloudFilterFixture, FilterMovedDownHierarchy)
{
    ...
    fs::rename(root(*cu) / ".megaignore",
               root(*cu) / "0" / ".megaignore");
...
public:
    fs::path debrisFilePath(const string& path) const
    {
        return fs::path(".debris") / todaysDate() / path;
    }
...
TEST_F(CloudToLocalFilterFixture, DoesntMoveIgnoredNodes)
{
    ...
    ASSERT_TRUE(cdu->movenode("cdu/d/fx", "cdu"));
...
TEST_F(CloudToLocalFilterFixture, DoesntRenameIgnoredNodes)
{
    ...
    Node* node =
      cdu->drillchildnodebyname(cdu->gettestbasenode(), "cdu/x");
    ASSERT_TRUE(node);

    node->attrs.map['n'] = "y";
    ASSERT_TRUE(cdu->setattr(*node));
...
TEST_F(CloudToLocalFilterFixture, DoesntRubbishIgnoredNodes)
{
    ...
    // Remove cdu/x.
    ASSERT_TRUE(cdu->deleteremote("cdu/x"));
...
TEST_F(CloudToLocalFilterFixture, FilterMovedAcrossHierarchy)
{
    ...
    ASSERT_TRUE(cd->movenode("x/a/.megaignore", "x/b"));
...
TEST_F(CloudToLocalFilterFixture, FilterMovedDownHierarchy)
{
    ...
    ASSERT_TRUE(cd->movenode("x/.megaignore", "x/a"));
...
TEST_F(CloudToLocalFilterFixture, FilterMovedUpHierarchy)
{
    ...
    ASSERT_TRUE(cd->movenode("x/a/.megaignore", "x"));
